/* B3 gate (decision-level). The device and the server run the SAME int8 graph, but a
   ...
   1.27 vs onnxruntime-node 1.24 vs onnxruntime-web in the browser): measured cosine ~0.994
   mean / ~0.98 min between runtimes, tokenisation identical. With heads trained on int8
   embeddings (A4) that residual is decision-safe: 0 flips / 28, |Δrisk| <= 0.055, tag-set
   Jaccard 0.977. Bit-level parity needs static (calibrated) quantisation -- PLAN follow-up. Skipped when the self-hosted model files
...
test("int8 ONNX in Node agrees with Python-ONNX at the decision level (same graph, different ORT)", { skip: !available && "self-hosted model not downloaded" }, async () => {
  ...
  assert.ok(mean >= 0.99, `mean cosine ${mean}`);
  assert.ok(min >= 0.97, `min cosine ${min}`);
  ...
  assert.ok(maxDelta < 0.1, `max risk delta ${maxDelta}`);
  assert.equal(flips, 0);
  assert.ok(jaccard >= 0.95, `tag-set Jaccard ${jaccard}`);
});
