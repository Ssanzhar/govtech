...
def test_synthesize_produces_requested_count_of_incidents():
    incidents = synthesize_incidents(_FAMILIES, count=60, seed=42, start_time=_START, hmac_key=TEST_HMAC_KEY)
...
def test_each_incident_belongs_to_a_family_with_matching_transcript_and_number():
    ...
    for inc in incidents:
        ...
        assert inc.number_hash in {hashed(n) for n in fam.phone_numbers}
        assert inc.number_prefix == prefix(fam.phone_numbers[0])
        assert inc.label.risk >= 0.5  # incidents are scams


def test_synthesis_is_deterministic_for_a_seed():
    a = synthesize_incidents(_FAMILIES, count=40, seed=7, start_time=_START, hmac_key=TEST_HMAC_KEY)
    b = synthesize_incidents(_FAMILIES, count=40, seed=7, start_time=_START, hmac_key=TEST_HMAC_KEY)
    assert [(i.id, i.script_family, i.number_hash, i.transcript) for i in a] == [
        (i.id, i.script_family, i.number_hash, i.transcript) for i in b
    ]


def test_different_seed_changes_assignment():
    a = synthesize_incidents(_FAMILIES, count=60, seed=1, start_time=_START, hmac_key=TEST_HMAC_KEY)
    b = synthesize_incidents(_FAMILIES, count=60, seed=2, start_time=_START, hmac_key=TEST_HMAC_KEY)
    assert [i.script_family for i in a] != [i.script_family for i in b]


def test_timestamps_within_window():
    incidents = synthesize_incidents(_FAMILIES, count=60, seed=42, start_time=_START, hmac_key=TEST_HMAC_KEY, span_days=30.0)
...
def test_novel_family_is_represented_and_flagged_via_script_family():
    incidents = synthesize_incidents(_FAMILIES, count=120, seed=42, start_time=_START, hmac_key=TEST_HMAC_KEY)
    novel = [i for i in incidents if i.script_family == "crypto_giveaway_new"]
    assert len(novel) >= 1


def test_larger_weight_gets_more_incidents():
    incidents = synthesize_incidents(_FAMILIES, count=300, seed=42, start_time=_START, hmac_key=TEST_HMAC_KEY)
...
def test_incident_ids_are_unique():
    incidents = synthesize_incidents(_FAMILIES, count=100, seed=42, start_time=_START, hmac_key=TEST_HMAC_KEY)
...
def test_empty_families_raises():
    with pytest.raises(ValueError):
        synthesize_incidents((), count=10, seed=1, start_time=_START, hmac_key=TEST_HMAC_KEY)


def test_non_positive_count_raises():
    with pytest.raises(ValueError):
        synthesize_incidents(_FAMILIES, count=0, seed=1, start_time=_START, hmac_key=TEST_HMAC_KEY)
