...
def _incident(iid, text, number):
    return Incident(id=iid, dialogue_id=iid, transcript=text, label=Label(risk=0.9), phone_number=number)
...
def _fixture():
    ...
    incidents = []
    for i in range(8):
        incidents.append(_incident(f"a{i}", "банк код из смс", "n_a"))
    for i in range(8):
        incidents.append(_incident(f"b{i}", "следователь дело финпол", "n_b"))
    for i in range(2):
        incidents.append(_incident(f"x{i}", "крипто раздача удвоим баланс", "n_x"))
    return incidents
