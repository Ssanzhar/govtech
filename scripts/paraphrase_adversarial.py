...
Run: `python scripts/paraphrase_adversarial.py [--max-attempts 3] [--limit N]`
...
def main(argv: list[str] | None = None) -> int:
    ...
    for index, source in enumerate(sources, 1):
        try:
            produced.append(paraphrase_dialogue(source, lexicon, paraphrase=paraphrase, max_attempts=args.max_attempts))
            status = "ok"
    ...
    (SOURCE_DIR / f"{ADVERSARIAL_SPLIT}.jsonl").write_text(payload, encoding="utf-8")
    (processed / f"{ADVERSARIAL_SPLIT}.jsonl").write_text(payload, encoding="utf-8")
    prompt_hash = hashlib.sha256(build_paraphrase_prompt(sources[0], lexicon).encode("utf-8")).hexdigest() if sources else None
    manifest = {
    ...
    print(f"produced {len(produced)}, failed {len(failures)} -> {SOURCE_DIR / (ADVERSARIAL_SPLIT + '.jsonl')} (+ processed copy)")
    return 0
